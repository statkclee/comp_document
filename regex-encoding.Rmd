...
## 사람과 기호 그리고 컴퓨터 [^pragmatic-unicode] [^korean-encoding] {#encoding-korean}
...
# 영어권에서 바라본 유니코드와 `utf-8` [^swcarpentry-v4-encoding] {#utf-8-from-europe}

[^swcarpentry-v4-encoding]: [파이썬 - 텍스트](http://v4.software-carpentry.org/python/text.html)

## 아스키(ASCII) 코드 {#utf-8-ascii}
...
```{bash iconv-utf-8, eval = FALSE}
$ iconv -f UTF8 -t EUC-KR sms_db_master.csv > sms_db_master_encoding.csv
```


## R 인코딩 확인 {#r-encoding-check}

`Sys.getlocale()` 명령어를 통해 현재 설정된 인코딩을 확인한다. 맥에서는 다음과 같이 확인할 수 있다.

```{r check-local-system}
library(tidyverse)

# Sys.getlocale()

stringr::str_split(Sys.getlocale(), "/") %>% unlist
```

윈도우 운영체제를 사용하는 경우 
`CP949`가 기본디폴트 설정으로 떨어진다. 
윈도우의 경우 다음과 같은 결과가 출력된다.

```{r encoding-windows, eval = FALSE}
# 윈도우즈
...
```{r check-localization}
l10n_info()
```

윈도우의 경우 다음과 같이 결과가 출력된다.

```{r check-localization-windows, eval = FALSE}
$MBCS
[1] TRUE
...
윈도우에서 확인했을 때 다음과 같이 확인된다.

```{r check-platform-windows, eval = FALSE}
...
```{r check-set-locale-windows, eval = FALSE}
Sys.setlocale("LC_COLLATE", "ko_KR.UTF-8")
```


## `readr` 인코딩 알아내기 {#readr-encoding}
...
한글 Windows의 메모장의 ANSI 인코딩이란, CP949 (Code Page 949)을 일컬으며, `CP949`는 `EUC-KR`의 확장으로 하위 호환성이 있다.

```{r guess-encoding-euc-kr}
library(readr)
guess_encoding("data/iris_euc_kr.csv", n_max = 100)
```

```{r guess-encoding-euc-kr-output, eval = FALSE}
    encoding confidence
1     EUC-KR       1.00
2     EUC-JP       0.97
3 ISO-8859-2       0.38
...
```{r guess-encoding-utf-8}
guess_encoding("data/iris_utf_8.csv", n_max = 100)
```

```{r guess-encoding-utf-8-output, eval = FALSE}
      encoding confidence
1        UTF-8       1.00
2 windows-1250       0.42
3 windows-1252       0.24
...
## `stringi` 인코딩 확인 {#stringi-encoding}

`stringi` 팩키지 `stri_enc_detect()` 함수로 인코딩을 확인할 수 있다.

<div class = "row">
  <div class = "col-md-6">
**EUC-KR 인코딩**

```{r stringi-detect-stringi}
euc_kr_bytes <- readBin("data/iris_euc_kr.csv", what="raw",n=200)
stringi::stri_enc_detect(euc_kr_bytes)
```
...
```{r stringi-detect-utf-8}
utf_8_bytes <- readBin("data/iris_utf_8.csv", what="raw",n=200)
stringi::stri_enc_detect(utf_8_bytes)
```
...
# 한글 윈도 파일 읽어오기  {#r-unicode-korea}

인코딩이 맞니 않는 한글을 그냥 읽어들일 경우 다음과 같은 오류가 발생되어 읽어 들일 수가 없다.

```{r korean-readr, error=TRUE}
readr::read_csv("data/iris_euc_kr.csv") 
```


먼저 `read.csv()` 함수를 사용해서 `fileEncoding=`을 지정하여 제대로 윈도우 한글 파일을 읽어들일 수 있도록 한다.

<div class = "row">
  <div class = "col-md-6">
**`ISO-8859-1`**

```{r korean-readr-encoding-fileEncoding1}
read.csv("data/iris_euc_kr.csv", 
...
```{r korean-readr-encoding-fileEncoding2}
read.csv("data/iris_euc_kr.csv", 
         fileEncoding = "EUC-KR") %>% 
  head
```
...
`tidyverse` 생태계를 구성하는 `readr` 팩키지 `read_csv()` 함수를 사용해서 한글 윈도우 인코딩 파일을 읽어 들이도록 한다.

<div class = "row">
  <div class = "col-md-6">
**`ISO-8859-1`**

```{r korean-readr-encoding-euc-kr, error=TRUE}
library(tidyverse)

# ISO-8859-1
confirmed_encoding <- readr::guess_encoding("data/iris_euc_kr.csv") %>%
  top_n(1, wt=confidence) %>%
  pull(encoding)

read_csv("data/iris_euc_kr.csv", 
...
```{r korean-readr-encoding-, error=TRUE}
read_csv("data/iris_euc_kr.csv", locale = 
