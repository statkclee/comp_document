...
subtitle: "정적 웹콘텐츠 제작환경 - 제킬(`jekyll`)"
output:
  html_document: 
    toc: yes
    toc_float: true
...
정적 웹콘텐츠 개발 툴체인(Tool Chain)을 이해하고, 
정적 웹콘테츠 개발환경을 루비언어 제킬을 활용하여 구축한다.

# 정적 웹콘텐츠 제작 툴체인(Tool Chain) {#static-webcontents}
...
``` {r web-contents-pandoc, eval=FALSE}
...
### Pandoc 최신 설치 {#web-contents-pandoc}
...
``` {r web-contents-pandoc-advanced, eval=FALSE}
...
``` {r web-contents-path-check, eval=FALSE}
export PATH=$HOME/.cabal/bin:$PATH
```

## 루비(Ruby) 설치 {#ruby-install}
...
### 깨끗한 우분투 설치 환경 만들기 {#ruby-install-linux}  

RVM을 설치하기 전에 깨끗하게 우분투에 있는 이전버젼 `ruby`, `ruby-rvm`을 제거한다.

``` {r web-contents-ruby, eval=FALSE}
...
### `rdoc`와 `ri` 문서화 모듈 없이 설치 {#ruby-without-rdoc-ri}
...
``` {r web-contents-ruby-rdoc, eval=FALSE}
...
``` {r web-contents-install-rvm, eval=FALSE}
root@std-ubuntu:~# sudo apt-get install -y git build-essential curl wget
```

### `rvm` 통한 `ruby` 설치 및 환경설정 {#ruby-rvm-install}
...
``` {r web-contents-echo, eval=FALSE}
...
``` {r web-contents-ruby-version-check, eval=FALSE}
...
``` {r web-contents-jekyll, eval=FALSE}
...
# 정적 웹콘텐츠 제작 테스트 {#make-static-webcontents}

마크다운으로 저작을 한 후에 HTML을 비롯한 다양한 콘텐츠 형식으로 변환이 되는지 테스트를 하기 위해서 먼저 GitHub에서 예제 파일을 하나 다운로드한다. 마크다운 `.md` 파일을 `.html` 파일로 변환한다.


``` {r web-contents-make-static, eval=FALSE}
...
``` {r web-contents-make-static-jekyll, eval=FALSE}
...
## 변경한 경로명(PATH) 적용 {#ubuntu-path-apply}
...
``` {r web-contents-source-apply, eval=FALSE}
