...
# 왜 `blogdown`을 사용해야 하나? [^arm-blog] {-#why-blogdown}

[^arm-blog]: [`rstudio::conf 2019`, "Advanced R Markdown Workshop"](https://arm.rbind.io/)

`WordPress`, `Tumblr`, `Medium.com`, `Blogger.com`은 R 마크다운 지원이 전무하고, 수학 수식도 없거나 지원도 그다지 만족스럽지 않다. 
이와 비교하여 휴고(Hugo)를 사용할 경우 모든 웹사이트를 구성하는 파일이 정적 파일이라 PHP 혹은 데이터베이스도 필요없으며 로그인도 필요없고, 오프라인을 포함하여 어디에서나 잘 지원된다.
특히 서버쪽에 컴퓨팅이 필요하지 않아 상당히 빠르고 CDN을 통해 속도도 매우 빠르다.

데이터 사이언스 블로그를 만들어 인터넷에 퍼블리싱하기 위해서 다음과 같은 구성요소가 필요하다.

1. Hugo: 정적 웹사이트 구축을 위해 필요한 프레임워크
1. `blogdown`: Hugo를 R마크다운과 연결시켜 주는 역할
1. GitHub: 버전 제어 및 소스 코드 관리
1. Netlify: GitHub에서 변경내용을 빌드하고 배포는 물론이고 블로그를 호스팅하는 역할을 수행.  

![데이터 사이언스 블로그 퍼블리싱 개요](fig/blogdown-hugo-netlify.png){#id .class width="100%"}

# `blogdown` 팩키지  [^arm-blogdown] [^blogdown-allison] {#blogdown}

[^blogdown-allison]: [Alison Hill (Jun 12, 2017), "Up & Running with blogdown"](https://alison.rbind.io/post/2017-06-12-up-and-running-with-blogdown/)

[^arm-blogdown]: [Alison Hill | rstudio::conf | 2019-01-15, "Making Websites in R Markdown"](https://arm.rbind.io/slides/blogdown.html#1)
...
![`blogdown` 개요](fig/blogdown-overview.png){#id .class width="77%"}
...
1. `blogdown` 작업의 시작은 `GitHub`에 저장소를 만드는 것부터 시작된다. 
    - 예를 들어, <https://github.com/statkclee/github_blog>
1. `Github` 저장소를 클로하여 로컬 컴퓨터에서 작업한다.
1. `blogdown` 팩키지를 사용해서 휴고를 설치한 후, 블로그 사이트를 생성하고, 블로그 글을 작성한다.
    - `install_hugo()`
    - `new_site()`
    - `new_post()`
1. `netfify.com`웹사이트에 블로그 사이트를 배포한다.
    - `GitHub` 저장소를 `netlify`사이트에 연결시켜 두면 로컬 컴퓨터에서 작성한 블로그 포스트를 `netlify` 블로그 호스팅 서버에서 빌드 및 배포가 되어 무척이나 편리하다.
    - <https://tidyverse-korea.netlify.com/>
    
![`blogdown` 작업흐름도](fig/blogdown-workflow.png){#id .class width="100%"}

## `blogdown` 설치 {#install-blogdown}
...
# 정적 웹사이트와 동적 웹사이트 비교[^static-dynamic-comparison] {#static-web-vs-dynamic-web} 
...
# 정적 웹콘텐츠 블로그 배포 [^blogdown-github] {#blog-deployment} 
...
## IDC 혹은 AWS 클라우드 배포 {#aws-blogdown-deployment}
